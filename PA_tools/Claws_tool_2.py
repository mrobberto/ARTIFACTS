...
data = ascii.read("Claws_table.txt")  
Mod_A = data[:][0:13]
Mod_B = data[:][13:]
...
Max_corners_V2_arcsec = np.max([ModA_corners_V2_arcsec,ModB_corners_V2_arcsec])
Min_corners_V2_arcsec = np.min([ModA_corners_V2_arcsec,ModB_corners_V2_arcsec])
Max_corners_V3_arcsec = np.max([ModA_corners_V3_arcsec,ModB_corners_V3_arcsec])
Min_corners_V3_arcsec = np.min([ModA_corners_V3_arcsec,ModB_corners_V3_arcsec])
Center_field_V2_arcsec = (Min_corners_V2_arcsec + Max_corners_V2_arcsec) / 2
Center_field_V3_arcsec = (Min_corners_V3_arcsec + Max_corners_V3_arcsec) / 2
#
#
Center_field_SkyCoord_deg = SkyCoord(Center_field_V2_arcsec*u.arcsec,Center_field_V3_arcsec*u.arcsec)
#
...
corners_Arev_deg = SkyCoord( -Mod_A[0][:]  ,Mod_A[1][:],frame='icrs',unit="arcsec")
corners_Brev_deg = SkyCoord( -Mod_B[0][:]  ,Mod_B[1][:],frame='icrs',unit="arcsec")
#fix the PAs: 359deg must be -1 etc.
i_neg_A = [i for i in range(len(corners_Arev_deg)) if corners_Arev_deg.ra.value[i] > 180.]
corners_Arev_deg.ra.value[i_neg_A] -= 360
i_neg_B = [i for i in range(len(corners_Brev_deg)) if corners_Brev_deg.ra.value[i] > 180.]
corners_Brev_deg.ra.value[i_neg_B] -= 360
#
corners_Arev_deg_V2V3 = ICRS(corners_Arev_deg.ra.value*u.deg,corners_Arev_deg.dec.value*u.deg)
corners_Arev_deg_LocalFrame = corners_Arev_deg_V2V3.transform_to(LocalFrame)
corners_Brev_deg_V2V3 = ICRS(corners_Brev_deg.ra.value*u.deg,corners_Brev_deg.dec.value*u.deg)
corners_Brev_deg_LocalFrame = corners_Brev_deg_V2V3.transform_to(LocalFrame)
...
PA_corners_A_rad = position_angle(0,0,corners_Arev_deg.ra,corners_Arev_deg.dec)
PA_corners_B_rad = position_angle(0,0,corners_Brev_deg.ra,corners_Brev_deg.dec)
#
#fix the PAs: 359deg must be -1 etc.
i_neg_A = [i for i in range(len(PA_corners_A_rad)) if PA_corners_A_rad.value[i] > (np.pi/2)]
PA_corners_A_rad[i_neg_A] -= (2*np.pi*u.rad)
i_neg_B = [i for i in range(len(PA_corners_B_rad)) if PA_corners_B_rad.value[i] > (np.pi/2)]
...
votable = parse("stars_kmag1p5_2mass.vot")
...
i_found = [i for i in range(len(sep)) if (sep.degree[i]>min_radius and  sep.degree[i]<max_radius)]  
#
#find the PA of all sources in the corona
ra_0 = c1.ra.value ; dec_0=c1.dec.value
#
#
#PA wants units in radiants!
PA = position_angle(ra_0*np.pi/180.,dec_0*np.pi/180.,ra*np.pi/180.,dec*np.pi/180.)
print("For coordinates: RA_0 = {:10f} - DEC_0 = {:10f}".format(ra_0,dec_0))
...
v = (corners_Arev_deg_LocalFrame.data.lon.value[0],corners_Arev_deg_LocalFrame.data.lat.value[0])
for j in range(len(ModA_corners_V2_arcsec)): #loop on the poligon corners
    v=*v, (corners_Arev_deg_LocalFrame.data.lon.value[j],corners_Arev_deg_LocalFrame.data.lat.value[j])
vt_A = np.array([*v[2:]]) * 180/np.pi
polygon_A = Polygon(vt_A) 

v = (corners_Brev_deg_LocalFrame.data.lon.value[0],corners_Brev_deg_LocalFrame.data.lat.value[0])
for j in range(len(ModB_corners_V2_arcsec)): #loop on the poligon corners
    v=*v, (corners_Brev_deg_LocalFrame.data.lon.value[j],corners_Brev_deg_LocalFrame.data.lat.value[j])
...
for i in i_found:
    ...
    plt.plot(corners_Arev_deg_LocalFrame.data.lon.value*180/np.pi,corners_Arev_deg_LocalFrame.data.lat.value*180/np.pi + plot_yoffset)
    plt.plot(corners_Brev_deg_LocalFrame.data.lon.value*180/np.pi,corners_Brev_deg_LocalFrame.data.lat.value*180/np.pi + plot_yoffset)
    plt.title("Target "+radec_0,fontsize=20)
    plt.text(-2.8,2.6+plot_yoffset,"Bright star at   "+c2[i].to_string('hmsdms'),fontsize=16)
...
#DETAILED EDGE OF THE REGION
    ...
    print("{:10f} {:10f} {:10f} {:8.3f} {:6.3f} {:8.3f} {:8.3f}".format(ra[i],dec[i],sep.degree[i],PA.degree[i],Kmag[i],PA.degree[i]+min_PA_rad.degree,PA.degree[i]+max_PA_rad.degree))
...
#    print(sep.degree[i])
    ...
    for j in range(nsamples+1):
        ...
        x = r * np.sin(theta) * 180/np.pi  + Offset_V2_arcsec/3600.  #* np.cos(theta)
        ...
        point = Point(xy_deg_LocalFrame.lon.value,xy_deg_LocalFrame.lat.value)
        
        #CHECK IF THE POINT IS INSIDE THE A REGION
        isin_A.append(polygon_A.contains(point))
        print(j,x,y,polygon_A.contains(point),r*180/np.pi,theta.deg+PA.degree[i])
...
#        print(counter)
        if counter[-1] != counter_last:
            ...
            up = -up
            plt.text(xy_deg_LocalFrame.lon.value-0.3,plot_yoffset+1+up,"{:7.2f}".format(edge_rad.degree+PA.degree[i]),fontsize='16')
        counter_last = counter[-1]

        
        plt.plot(xy_deg_LocalFrame.lon.value,xy_deg_LocalFrame.lat.value+plot_yoffset,'ro')

    for ia in range(0,len(angle_to_avoid),2):
        txt = "avoid from: {:6.2f}deg to {:6.2f}deg".format(angle_to_avoid[ia]+PA.degree[i],angle_to_avoid[ia+1]+PA.degree[i])
    ...
    plt.savefig(output_dir+'/Target = '+c1.to_string('hmsdms')+'- Birght star = '+c2[i].to_string('hmsdms')+'.jpg')    
    plt.show()
